...
def search_user(email: str):
  user_data = conn.execute(user.select().where(
      user.c.email == email)).mappings().fetchone()
  return user_data
...
def callback(code: str):
  ...
  if not user_data:
    ...
    conn.commit()
    select = conn.execute(user.select().where(
        user.c.id_user == res.lastrowid)).mappings().fetchone()
